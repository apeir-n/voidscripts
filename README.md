...
`colorbaby` & `colorxpet`
- these are scripts that change the colors in the .xpm files used by [xpet](https://github.com/uint23/xpet)
- it injects some lines from a file generated by a pywal template into each xpm file (there are a lot)
- i might want to generalize the scripts into one at some point, but right now they're separate because one set of xpms needs 4 colors and the other needs 2
...
`launch_*`
- wrapper scripts for `startx`
- launches different window managers
...
`paper`
- uses pywal16 to set the wallpaper and run some other stuff
...
- anyway, the subdirs are `ls`'ed and piped into dmenu to be selected
...
`pather`
- just adds executable permissions to a file and symlinks it to my path
...
- images below:
    - listed in the order of prompt -a, -i, -o
...
- implementing the prompt as a function with local variables makes it way easier to read and modify
    ...
    - `prompta` is like `prompti` but with no icons
...
`sloppeg`
- a screen recorder with `ffmpeg` that takes a geometry specified by `slop`
...
- it just makes the update routine simpler
- it used to call `packup` optionally but i just ended up integrating those update commands into this script
